...
class TestLocalTextCategorizationDataset(unittest.TestCase):
    ...
    def test__get_num_samples_is_correct(self):
        ...
        pd.read_csv = MagicMock(return_value=pd.DataFrame({
            ...
            'tag_name': ['tag_a', 'tag_b', 'tag_a', 'tag_b', 'tag_a', 'tag_b'],
            'tag_id': [1, 2, 3, 4, 5, 6],
            'tag_position': [0, 0, 0, 0, 0, 0],
            'title': ['title_1', 'title_2', 'title_3', 'title_4', 'title_5', 'title_6']
        }))
        cat = utils.LocalTextCategorizationDataset('fileman', 1, min_samples_per_label=1)
    ...
    def test_get_test_batch_returns_expected_shape(self):
        ...
        cat = utils.LocalTextCategorizationDataset('fileman', 3, train_ratio=0.5, min_samples_per_label=1)
    ...
    def test_get_train_batch_raises_assertion_error(self):
        ...
        cat = utils.LocalTextCategorizationDataset('fileman', 1, train_ratio=0.5, min_samples_per_label=1)
        ini_batch_index = cat.train_batch_index
        first_batch = cat.get_train_batch()
        updated_batch_index = cat.train_batch_index
        self.assertEqual(updated_batch_index, ini_batch_index+1)
