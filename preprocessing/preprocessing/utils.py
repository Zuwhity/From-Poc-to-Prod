...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def get_train_batch(self):
        ...
        next_y = np.array(self.y_train[i * self.batch_size: (i + 1) * self.batch_size])
    ...
    def get_test_batch(self):
        """
        it does the same as get_test_batch for the test set
        """
        # TODO: CODE HERE
        ...
        next_y = np.array(self.y_test[i*self.batch_size: (i+1)*self.batch_size])
        # When we reach the max num batches, we start anew
